...
# 读取CSV文件
data = pd.read_csv('Output_GDP_CD.csv', dtype=object)
...
# 第一个子图：平均变化分数
ax1.bar(top_10_score_change['Name'], top_10_score_change['Avg_Score_Change'], color='tab:blue', alpha=0.7, width=0.4)
ax1.set_xlabel('国家', fontproperties="SimHei")
ax1.set_ylabel('平均变化分数', fontproperties="SimHei")
ax1.tick_params(axis='x', rotation=90)

# 第二个子图：平均变化次数
ax2.bar(top_10_change_count['Name'], top_10_change_count['Avg_Change_Count'], color='tab:red', alpha=0.7, width=0.4)
ax2.set_xlabel('国家', fontproperties="SimHei")
ax2.set_ylabel('平均变化次数', fontproperties="SimHei")
ax2.tick_params(axis='x', rotation=90)

# 设置标题
fig.suptitle('平均变化分数和平均变化次数Top 10国家', fontsize=16, fontweight='bold', fontproperties="SimHei")
plt.show()
print(top_10_score_change)
print(top_10_change_count)
print('###############################################################################################################')
...
max_score_stats = max_score_countries.groupby('Name').describe()['Score'].loc[:, ['count', 'mean', 'std', 'min', 'max']]
